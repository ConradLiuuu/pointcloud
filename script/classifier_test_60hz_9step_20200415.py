...
class Listener:
    def __init__(self):
        ...
        self.__predction_balls_hat = cp.ones((1,self.__time_step,3))
        self.__predction_balls_hat2 = cp.ones((1,self.__time_step,3))
    ...
    def top5(self):
        ...
        with graph.as_default():
            self.__pred = self.__pred_top5.predict(cp.asnumpy(self.__arr_prediction.reshape(1,self.__time_step,3)), verbose=1)
            self.__predction_balls_hat = self.__pred_top5.predict(cp.asnumpy(self.__pred), verbose=1)
            self.__predction_balls_hat2 = self.__pred_top5.predict(cp.asnumpy(self.__predction_balls_hat), verbose=1)
        #print(self.__pred)
    def top6(self):
        print("top spin speed 6")
        with graph.as_default():
            self.__pred = self.__pred_top6.predict(cp.asnumpy(self.__arr_prediction.reshape(1,self.__time_step,3)), verbose=1)
            self.__predction_balls_hat = self.__pred_top6.predict(cp.asnumpy(self.__pred), verbose=1)
            self.__predction_balls_hat2 = self.__pred_top6.predict(cp.asnumpy(self.__predction_balls_hat), verbose=1)
        #print(self.__pred)
    def left5(self):
        print("left spin speed 5")
        with graph.as_default():
            self.__pred = self.__pred_left5.predict(cp.asnumpy(self.__arr_prediction.reshape(1,self.__time_step,3)), verbose=1)
            self.__predction_balls_hat = self.__pred_left5.predict(cp.asnumpy(self.__pred), verbose=1)
            self.__predction_balls_hat2 = self.__pred_left5.predict(cp.asnumpy(self.__predction_balls_hat), verbose=1)
        #print(self.__pred)
    def left6(self):
        print("left spin speed 6")
        with graph.as_default():
            self.__pred = self.__pred_left6.predict(cp.asnumpy(self.__arr_prediction.reshape(1,self.__time_step,3)), verbose=1)
            self.__predction_balls_hat = self.__pred_left6.predict(cp.asnumpy(self.__pred), verbose=1)
            self.__predction_balls_hat2 = self.__pred_left6.predict(cp.asnumpy(self.__predction_balls_hat), verbose=1)
        #print(self.__pred)
    def right5(self):
        print("right spin speed 5")
        with graph.as_default():
            self.__pred = self.__pred_right5.predict(cp.asnumpy(self.__arr_prediction.reshape(1,self.__time_step,3)), verbose=1)
            self.__predction_balls_hat = self.__pred_right5.predict(cp.asnumpy(self.__pred), verbose=1)
            self.__predction_balls_hat2 = self.__pred_right5.predict(cp.asnumpy(self.__predction_balls_hat), verbose=1)
        #print(self.__pred)
    def right6(self):
        print("right spin speed 6")
        with graph.as_default():
            self.__pred = self.__pred_right6.predict(cp.asnumpy(self.__arr_prediction.reshape(1,self.__time_step,3)), verbose=1)
            self.__predction_balls_hat = self.__pred_right6.predict(cp.asnumpy(self.__pred), verbose=1)
            self.__predction_balls_hat2 = self.__pred_right6.predict(cp.asnumpy(self.__predction_balls_hat), verbose=1)
        #print(self.__pred)
    def back5(self):
        print("back spin speed 5")
        with graph.as_default():
            self.__pred = self.__pred_back5.predict(cp.asnumpy(self.__arr_prediction.reshape(1,self.__time_step,3)), verbose=1)
            self.__predction_balls_hat = self.__pred_back5.predict(cp.asnumpy(self.__pred), verbose=1)
            self.__predction_balls_hat2 = self.__pred_back5.predict(cp.asnumpy(self.__predction_balls_hat), verbose=1)
        #print(self.__pred)
    def back6(self):
        print("back spin speed 6")
        with graph.as_default():
            self.__pred = self.__pred_back6.predict(cp.asnumpy(self.__arr_prediction.reshape(1,self.__time_step,3)), verbose=1)
            self.__predction_balls_hat = self.__pred_back6.predict(cp.asnumpy(self.__pred), verbose=1)
            self.__predction_balls_hat2 = self.__pred_back6.predict(cp.asnumpy(self.__predction_balls_hat), verbose=1)
        #print(self.__pred)
    ...
    def find_hitting_point_v2(self):
        ...
        if (float(cp.min(self.__pred)) < -45):
            index = cp.argmin(self.__pred)
            row = (int(index)-1) / 3
        ...
        elif (-45 < float(cp.min(self.__pred)) < -40):
            index = cp.argmin(self.__pred)
            row = (int(index)-1) / 3
            print("count down ", int(row))
            #print("pred = ", self.__pred[:, row, :])
    ...
    def traj_C_possible_point(self):
        self.__predction_balls_hat2 = cp.array(self.__predction_balls_hat2)
        if (float(cp.min(self.__predction_balls_hat2)) < -45):
            index = cp.argmin(self.__predction_balls_hat2)
            row = (int(index)-1) / 3
            print("count down from traj C ", int(row)+18)
            if (row == 0):
                self.__predction_balls_hat = cp.array(self.__predction_balls_hat)
        ...
        elif (-45 < float(cp.min(self.__predction_balls_hat2)) < -40):
            index = cp.argmin(self.__predction_balls_hat2)
            row = (int(index)-1) / 3
            print("count down from traj C ", int(row)+18)
            if (row == 0):
    ...
    def traj_B_possible_point(self):
        self.__predction_balls_hat = cp.array(self.__predction_balls_hat)
        if (float(cp.min(self.__predction_balls_hat)) < -45):
            index = cp.argmin(self.__predction_balls_hat)
            row = (int(index)-1) / 3
        ...
        elif (-45 < float(cp.min(self.__predction_balls_hat)) < -40):
            index = cp.argmin(self.__predction_balls_hat)
            row = (int(index)-1) / 3
            print("count down from traj B ", int(row)+9)
            if (row == 0):
